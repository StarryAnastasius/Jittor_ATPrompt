    ...
    def read_split(filepath, path_prefix):
        def _convert(items):
            ...
            for impath, label, classname in items:
                # 修复路径分隔符问题
                impath_clean = impath.replace("\\", "/")
                impath_clean = os.path.normpath(impath_clean)
                impath_full = os.path.join(path_prefix, impath_clean)
                impath_full = os.path.normpath(impath_full)
