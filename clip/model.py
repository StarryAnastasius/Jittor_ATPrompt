...
class ResidualAttentionBlock_IVLP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: jt.Var = None, add_prompt=False,
        ...
        if self.add_prompt:
            ...
            jt.nn.init.gauss_(ctx_vectors, std=0.02)
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: jt.Var = None, design_details=None,
        ...
        super().__init__()
        self.attn = attention.MultiheadAttention(d_model, n_head)  # Jittor MultiheadAttention
        self.ln_1 = LayerNorm(d_model)
        self.mlp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, d_model * 4)),
            ("gelu", QuickGELU()),
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: jt.Var = None, prompts_needed=0,
        ...
        if current_trainer in ['IVLP', 'VPT']:
            ...
            assert current_trainer in ['CoOp', 'CoCoOp'], f"不支持的Trainer类型: {current_trainer}"
            self.resblocks = nn.Sequential(*[
...
class VisionTransformer(nn.Module):
    ...
    def execute(self, x: jt.Var):
        ...
        # 拼接类嵌入
        class_emb = self.class_embedding.astype(x.dtype) + jt.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype)
        x = jt.cat([class_emb, x], dim=1)
        # 加位置嵌入
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        self.VPT_shallow = True  # MaPLe默认启用浅提示
    ...
    def execute(self, x: jt.Var, shared_ctx, compound_deeper_prompts):
        # patch嵌入
        x = self.conv1(x)
        x = x.reshape(x.shape[0], x.shape[1], -1).permute(0, 2, 1)
        # 拼接类嵌入
        ...
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)
        outputs = self.transformer([x, compound_deeper_prompts, 0])  # 第三个参数为counter初始值
        x = outputs[0]
        x = x.permute(1, 0, 2)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # 初始化视觉编码器（ResNet或VisionTransformer）
    ...
    def initialize_parameters(self):
        # 文本嵌入初始化
        jt.nn.init.gauss_(self.token_embedding.weight, std=0.02)
        jt.nn.init.gauss_(self.positional_embedding, std=0.01)
        ...
        if isinstance(self.visual, ModifiedResNet) and self.visual.attnpool is not None:
            ...
            for proj in [self.visual.attnpool.q_proj, self.visual.attnpool.k_proj,
                ...
                jt.nn.init.gauss_(proj.weight, std=std)

        # 残差块BN层初始化（最后一个BN权重置0）
        if isinstance(self.visual, ModifiedResNet):
            for resnet_block in [self.visual.layer1, self.visual.layer2, self.visual.layer3, self.visual.layer4]:
                for name, param in resnet_block.named_parameters():
                    if name.endswith("bn3.weight"):
                        jt.nn.init.zeros_(param)
        ...
        for block in self.transformer.resblocks:
            jt.nn.init.gauss_(block.attn.in_proj_weight, std=attn_std)
            jt.nn.init.gauss_(block.attn.out_proj.weight, std=proj_std)
            jt.nn.init.gauss_(block.mlp.c_fc.weight, std=fc_std)
            jt.nn.init.gauss_(block.mlp.c_proj.weight, std=fc_std)

        # 文本投影层初始化
        if self.text_projection is not None:
            jt.nn.init.gauss_(self.text_projection, std=self.transformer.width ** -0.5)

    def build_attention_mask(self):
        # 文本因果注意力掩码（下三角可见，上三角为-inf）
        mask = jt.empty(self.context_length, self.context_length)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # 上三角置1（填充-inf），下三角（含对角线）为0
        return mask

    @property
    def dtype(self):
        # 统一模型数据类型（跟随视觉编码器conv1权重类型）
    ...
    def encode_text(self, text):
        # 文本特征编码
        x = self.token_embedding(text).astype(self.dtype)  # [B, T, D]
        x = x + self.positional_embedding.astype(self.dtype)  # 加位置嵌入
        x = x.permute(1, 0, 2)  # [T, B, D]（适配Transformer输入）
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # [B, T, D]
        x = self.ln_final(x).astype(self.dtype)

        # 提取EOT（文本结束符）特征（取每个序列的最大索引对应特征）
        eot_indices = text.argmax(dim=-1)  # [B]
        x = x[jt.arange(x.shape[0]), eot_indices]  # [B, D]
    ...
    def execute(self, image, text):
        ...
        logits_per_image = logit_scale * image_features @ text_features.t()  # [B_img, B_text]
        logits_per_text = logit_scale * text_features @ image_features.t()  # [B_text, B_img]

        return logits_per_image, logits_per_text


def convert_weights(model: nn.Module):
    """将模型参数统一转为float32（替换原半精度转换）"""
    ...
    def _convert_weights_to_fp32(l):
        ...
        # MultiheadAttention参数转为float32
        if isinstance(l, attention.MultiheadAttention):
            for attr in [*[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]], "in_proj_bias", "bias_k", "bias_v"]:
                if hasattr(l, attr):
                    tensor = getattr(l, attr)
                    if tensor is not None:
                        tensor.data = tensor.data.astype('float32')
        ...
        for name in ["text_projection", "proj"]:
            if hasattr(l, name):
                attr = getattr(l, name)
                if attr is not None:
                    attr.data = attr.data.astype('float32')

    model.apply(_convert_weights_to_fp32)


def build_model(state_dict: dict, design_details):
    """从权重字典构建Jittor CLIP模型（带ATPrompt）"""
    # 判断是否为VisionTransformer
    vit = "visual.proj" in state_dict
    print(f'Build model: Vit architecture = {vit}')

    if vit:
        # VisionTransformer参数解析
        vision_width = state_dict["visual.conv1.weight"].shape[0]
        # 统计Transformer层数（通过注意力层参数）
        vision_layers = len([
            k for k in state_dict.keys() if k.startswith("visual.") and k.endswith(".attn.in_proj_weight")
        ])
        vision_patch_size = state_dict["visual.conv1.weight"].shape[-1]
        # 计算图像分辨率（位置嵌入长度-1开平方 × patch大小）
        grid_size = round((state_dict["visual.positional_embedding"].shape[0] - 1) ** 0.5)
        image_resolution = vision_patch_size * grid_size
    else:
        # ResNet参数解析（统计每层残差块数量）
        counts = [
        ...
        # 计算图像分辨率（注意力池化位置嵌入长度-1开平方 × 32）
        output_width = round((state_dict["visual.attnpool.positional_embedding"].shape[0] - 1) ** 0.5)
        vision_patch_size = None
        assert output_width ** 2 + 1 == state_dict["visual.attnpool.positional_embedding"].shape[0], \
            "ResNet注意力池化位置嵌入维度不匹配"
    ...
    transformer_heads = transformer_width // 64  # 每个头64维
    ...
    try:
        ...
        print(f'Warning: 部分权重未找到（可能是提示参数）: {missing_keys}')
