...
from jittor.dataset import Sampler, SequentialSampler, Dataset
import jittor


class RandomSampler(Sampler):
    """自定义随机采样器，修正__len__逻辑"""
    def __init__(self, data_source):
        self.data_source = data_source
        self.num_samples = len(data_source)  

    def __iter__(self):
        # 生成0到num_samples-1的随机索引
        return iter(random.sample(range(self.num_samples), self.num_samples))

    def __len__(self):

        return self.num_samples

# 将普通列表包装为 Jittor Dataset 对象
class ListDataset(Dataset):
    def __init__(self, data_list):
        super().__init__()
        self.data_list = data_list  # 存储原始样本列表（如Datum对象列表）
...
def build_sampler(
    ...
    if isinstance(data_source, list):
        data_source = ListDataset(data_source)
