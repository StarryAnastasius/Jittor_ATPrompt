...
class Lighting:
    ...
    def __init__(self, alphastd, eigval, eigvec):
        ...
        # torch.Tensor → jt.Var
        self.eigval = jt.array(eigval)
        self.eigvec = jt.array(eigvec)

    def __call__(self, img):
        if self.alphastd == 0:
            return img

        # torch.new().resize_ → jt.zeros_like + reshape；torch.normal → jt.normal
        alpha = jt.normal(0, self.alphastd, size=(3,), dtype=img.dtype, device=img.device)

        # 张量操作逻辑完全不变，仅替换torch API为jt
        rgb = (
            self.eigvec.astype(img.dtype).clone().mul(
