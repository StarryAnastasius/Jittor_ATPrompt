...
class Prototypes(nn.Module):
    ...
    def forward(self, x, reverse=False):
        if reverse:
            x = self.revgrad(x)
        x = jt.misc.normalize(x, p=2, dim=1)
...
class CDAC(TrainerXU):
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        loss_x = nn.cross_entropy_loss(logit_x, label_x)
        ...
        prob_u, prob_us = nn.softmax(logit_u, dim=1), nn.softmax(logit_us, dim=1)
        ...
        prob_u, prob_us, prob_us2 =nn.softmax(
            logit_u, dim=1
        ), nn.softmax(
            logit_us, dim=1
        ),nn.softmax(
        ...
        pl_loss = (
            nn.cross_entropy_loss(logit_us2, max_idx, reduction='none') * mask
        ).mean()
        ...
        cons_loss = cons_multi * nn.mse_loss(prob_us, prob_us2)
    ...
    def get_similarity_matrix(feat, topk, device):
        ...
        sim_mat = jt.zeros((feat_d.shape[0], feat_d.shape[0]))
