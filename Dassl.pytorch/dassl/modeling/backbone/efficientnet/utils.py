...
class SwishImplementation(jt.Function):
    """Jittor 自定义自动求导函数，实现 Swish 激活（x * sigmoid(x)）"""

    @staticmethod
    def forward(ctx, i):
        """前向传播：计算 Swish 输出，并保存中间变量用于反向求导"""
        # 1. 计算 Swish 前向结果（i * sigmoid(i)）
        sigmoid_i = jt.sigmoid(i)
        result = i * sigmoid_i
        
        # 2. 保存中间变量（sigmoid(i) 和原始输入 i）到 ctx，供反向传播使用
        # Jittor 用 ctx.save_vars() 替代 PyTorch 的 ctx.save_for_backward()
        ctx.save_vars(sigmoid_i, i)
    ...
    def backward(ctx, grad_output):
        """反向传播：手动计算 Swish 的梯度"""
        # 1. 恢复前向保存的中间变量
        # Jittor 用 ctx.saved_vars 替代 PyTorch 的 ctx.saved_variables
        sigmoid_i, i = ctx.saved_vars
        
        # 2. 计算 Swish 的梯度（导数公式：sigmoid(i) + i * sigmoid(i) * (1 - sigmoid(i))）
        # 结合链式法则，乘以上游梯度 grad_output
        grad_i = grad_output * (sigmoid_i + i * sigmoid_i * (1 - sigmoid_i))
